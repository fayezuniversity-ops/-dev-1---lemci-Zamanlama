...
int findNextProcess(vector<Process>& processes, int currentTime) {
...
int main() {
    ...
    int total_cs_for_stats = 0; 

    while(completed_count < n) {
        int current_p_idx = findNextProcess(p, current_time);
        ...
        if (current_p_idx == -1) {
            ...
            if (timeline.empty() || timeline.back().process_id != p[current_p_idx].id) {
                 total_cs_for_stats++;
            }

            p[current_p_idx].start_time = current_time;
            p[current_p_idx].ct = current_time + p[current_p_idx].bt;
            ...
            timeline.push_back({p[current_p_idx].id, current_time, p[current_p_idx].ct});
            current_time = p[current_p_idx].ct;
        }
    }
    ...
    double cs_overhead = total_cs_for_stats * 0.001;
    ...
    cout << "\nTotal Context Switches: " << total_cs_for_stats << endl;
